...
-- создает врага в позиции камеры
	...
	if vehicle and team then
		...
		vehicle:SetNameFromScript("Enemy"..tostring(random(9999)))
...
function GetDifficultyLevel()
    ...
    local filePath = "data\\profiles\\" .. profileName .. "\\profile.xml"
    ...
    local startParam = string.find(content, 'Id="PP_DIFFICULTY_LEVEL"')
    if not startParam then
        return 0
    end

    local startValue = string.find(content, 'GAIParam_Value="', startParam)
    if not startValue then
        return 0
    end

    local endValue = string.find(content, '"', startValue + 16)
    if not endValue then
        return 0
    end

    local valueStr = string.sub(content, startValue + 16, endValue - 1)
    local difficultyLevel = tonumber(valueStr)

    LOG(difficultyLevel)
    LOG(profileName)

    return difficultyLevel or 0
end

function GetProfileName()
    local logFile = io.open("exmachina.log", "r")
    if not logFile then
        return nil
    end
    ...
    for line in logFile:lines() do
        local _, _, match = string.find(line, 'Profile "(.+)" set')
        if match then
            profileName = match
        end
    end
...
function table_tostring(val, indent, seen)
    indent = indent or ""
    seen = seen or {}
    ...
    if t == "table" then
        ...
        for k, v in pairs(val) do
            ...
            result = result .. next_indent .. keystr .. " = " .. table_tostring(v, next_indent, seen) .. ",\n"
        end
...
function GetSpeed(vehicle)
    ...
    local vel = vehicle:GetLinearVelocity()
    local x = vel.x
    local y = vel.y
    local z = vel.z

    local speedMagnitude = math.sqrt(x*x + y*y + z*z)

    local kmh = speedMagnitude * 3.6

    kmh = kmh + 0.5
    return tonumber(string.format("%d", kmh))
end